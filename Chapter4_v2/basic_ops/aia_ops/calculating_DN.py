...
def wavelength_shift(Tx, Ty, A=9e2, B=30e-1):
    '''
    ...
    return 8e2 * Tx**2 +30e-1*Tx        +8e2* Ty**2+      9e-1 * Ty        -0.016
...
def get_stddev(Tx,Ty,image):
    '''
    Calculate the standard deviation of the Gaussian function for all pixels in parallel
    simga as a function of Tx, Ty, and DN(image array)

    '''

    stddev=0.0195+2e2*Tx**2              +    8e-4*cp.sqrt(image+10)       #+4e-6*image
    # stddev=cp.full(image.shape,0.0285)
    return stddev
def calculating_DN(image,wavelength_list,dtype=cp.float64):
    '''
    ...
    Tx,Ty=select_Tx_Ty(image_shape_x, image_shape_y)
